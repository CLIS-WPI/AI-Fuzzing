...
# Version 5.3: Comprehensive analysis, diagnostic flags, summary CSV, and diverse plots.
...
import os
# from scipy import stats # Uncomment if you plan to add detailed statistical tests

# --- Configuration ---
# !!! UPDATE THIS TO YOUR LATEST CSV FILENAME from the main simulation script (e.g., v22 output) !!!
...
SIMULATION_ITERATIONS = 100 # Default, will be updated if possible from CSV
BS_LOCATIONS_2D = np.array([[0,0], [100,0], [50, 86.6]]) # From your simulation script
NUM_UES = 30 # Ensure this matches the simulation that generated the CSV
# Noise power in Watts, should match the value used in the simulation for (I+N) derivation
# BW = 13.68e6 for v23 script, but v22 script used 20e6. This should match the BW for the CSV being analyzed.
# Assuming the CSV "fuzzing_results_v23_1_small_fast_fixed.csv" was from a 20MHz simulation:
BANDWIDTH_FOR_ANALYSIS = 20e6 # IMPORTANT: Adjust if your CSV used a different bandwidth for noise calculation
...
FLAGS = {
    ...
    "suspiciously_high_95th_percentile_sinr_values_present": False, # Specifically for >60dB mean
    "rsrp_values_suspiciously_high": False, # For individual RSRPs > TX_POWER_DBM + reasonable_gain
    "rsrp_values_at_floor": False, # For RSRPs hitting the -200dBm floor
}
# Thresholds for flags
LOW_SINR_PERCENTILE_THRESHOLD = -20.0 
HIGH_SINR_PERCENTILE_THRESHOLD = 55.0 # Made slightly more aggressive for flagging
INDIVIDUAL_EXTREME_LOW_SINR_THRESHOLD = -30.0
INDIVIDUAL_EXTREME_HIGH_SINR_THRESHOLD = 65.0 
HIGH_HO_RATE_THRESHOLD = 15.0 
LOW_FAIRNESS_THRESHOLD_OVERALL = 0.3 # Slightly adjusted
SIGNIFICANCE_LEVEL_P_VALUE = 0.05
SUSPICIOUS_RSRP_THRESHOLD_DBM = 35.0 # TX_POWER_DBM + 5dB small antenna gain/LoS effect

def parse_list_string_column(series, column_name_for_debug="Unknown"):
    # (Robust parsing from v5.2)
    parsed_list = []
...
def load_and_preprocess_data(csv_filepath):
    # (Robust loading and parsing from v5.2)
    ...
    try:
        ...
        # Infer NUM_UES from the length of a parsed list column if possible
        if 'serving_cell_ids_str' in df.columns and not df['serving_cell_ids_str'].empty:
            try:
                ...
                NUM_UES = len(ast.literal_eval(first_valid_list))
                print(f"Inferred NUM_UES from 'serving_cell_ids_str': {NUM_UES}")
            except StopIteration:
                print(f"Could not infer NUM_UES from 'serving_cell_ids_str', using default: {NUM_UES}")
            except (ValueError, SyntaxError):
                 print(f"Error parsing 'serving_cell_ids_str' to infer NUM_UES, using default: {NUM_UES}")

    except Exception as e:
        print(f"Error loading CSV: {e}"); FLAGS["data_loading_error"] = True; return None
    str_list_cols = ['cell_loads_list_str', 'ue_locations_str', 'serving_cell_ids_str', 'assigned_rsrp_list_str', 'assigned_sinr_list_str', 'fuzzed_load_modifier_str', 'fuzzed_pos_modifier_str', 'vulnerabilities']
    ...
    if 'vulnerabilities' in df.columns:
        df['vulnerabilities'] = df['vulnerabilities'].apply(lambda x: [str(v) for v in x] if isinstance(x, list) else ([] if pd.isna(x) else [str(x)]))
        df['vulnerability_count'] = df['vulnerabilities'].apply(len)
    else: df['vulnerability_count'] = 0
    numeric_metrics_to_check = ['avg_overall_sinr', 'avg_high_prio_sinr', 'fairness_index', 'handover_count_iter', 'num_ues_below_qos', 'sinr_5th_percentile', 'sinr_50th_percentile', 'sinr_95th_percentile', 'load_min', 'load_max', 'load_std']
...
def analyze_vulnerabilities(df, output_dir):
    ...
    os.makedirs(output_dir, exist_ok=True); print("\n--- Vulnerability Analysis ---")
    ...
    if vuln_counts:
        plt.figure(figsize=(14, 8)); simplified_vuln_map = {}
        for v_str, count in vuln_counts.items(): main_type = v_str.split(':')[0].strip(); simplified_vuln_map[main_type] = simplified_vuln_map.get(main_type, 0) + count
        if simplified_vuln_map:
            sns.barplot(x=list(simplified_vuln_map.keys()), y=list(simplified_vuln_map.values())); plt.title("Overall Distribution of Main Vulnerability Types")
            plt.ylabel("Total Count"); plt.xlabel("Main Vulnerability Type"); plt.xticks(rotation=30, ha="right"); plt.tight_layout()
    ...
    if 'fuzzer_type' in df.columns and 'vulnerability_count' in df.columns:
        plt.figure(figsize=(8, 6)); df.groupby('fuzzer_type')['vulnerability_count'].sum().plot(kind='bar')
        plt.title("Total Vulnerability Count by Fuzzer Type"); plt.ylabel("Total Vulnerabilities Detected"); plt.xlabel("Fuzzer Type")
        plt.xticks(rotation=0); plt.tight_layout(); plt.savefig(os.path.join(output_dir, "vuln_count_by_fuzzer.png")); print(f"Saved plot: {os.path.join(output_dir, 'vuln_count_by_fuzzer.png')}"), plt.close()
    if all(col in df.columns for col in ['scenario', 'algorithm', 'fuzzer_type', 'vulnerability_count']):
        try:
            ...
            if not grouped_vulns.empty:
                ...
                plt.tight_layout(rect=[0,0,0.9,0.95]); plt.savefig(os.path.join(output_dir, "vuln_count_by_algo_fuzzer_scenario.png")); print(f"Saved plot: {os.path.join(output_dir, 'vuln_count_by_algo_fuzzer_scenario.png')}"), plt.close()
...
def analyze_key_metrics(df, output_dir):
    ...
    for metric in key_numerical_metrics:
        if metric not in df.columns: print(f"Metric '{metric}' not found."); continue
        print(f"\nAnalyzing metric: {metric}")
        try:
            df[metric] = pd.to_numeric(df[metric], errors='coerce')
            if df[metric].isnull().all(): print(f"All values for metric '{metric}' are NaN."); continue
                    ...
                    if valid_sinrs_np.size > 0:
                        ...
                        for ue_idx in min_sinr_ue_indices[:1]:
                             ...
                             if isinstance(assigned_rsrps, list) and ue_idx < len(assigned_rsrps) and isinstance(assigned_rsrps[ue_idx], (int, float)) and pd.notna(assigned_rsrps[ue_idx]):
                                ...
                                if assigned_rsrps[ue_idx] <= -199.0: FLAGS["rsrp_values_at_floor"] = True # Check if RSRP is at floor
                ...
                if isinstance(assigned_sinrs, list) and assigned_sinrs:
                    ...
                    if valid_sinrs_np.size > 0:
                        ...
                        for ue_idx in max_sinr_ue_indices[:1]:
                            ...
                            if isinstance(assigned_rsrps, list) and ue_idx < len(assigned_rsrps) and isinstance(assigned_rsrps[ue_idx],(int,float)) and pd.notna(assigned_rsrps[ue_idx]):
                                print(f"        UE {ue_idx} Assigned RSRP: {assigned_rsrps[ue_idx]:.2f} dBm")
                                if assigned_rsrps[ue_idx] > SUSPICIOUS_RSRP_THRESHOLD_DBM: FLAGS["rsrp_values_suspiciously_high"] = True
                                signal_linear = 10**((assigned_rsrps[ue_idx]-30)/10)
                                sinr_val_ue = assigned_sinrs[ue_idx] if isinstance(assigned_sinrs[ue_idx], (int,float)) and pd.notna(assigned_sinrs[ue_idx]) else np.nan
                                if pd.notna(sinr_val_ue):
                                    sinr_linear = 10**(sinr_val_ue/10)
...
def plot_cdfs(df, output_dir):
    if df is None: print("CDF plotting skipped: DataFrame is None."); return
    print("\n--- Plotting CDFs for Key SINR Metrics ---"); os.makedirs(output_dir, exist_ok=True)
    sinr_metrics_for_cdf = ['avg_overall_sinr', 'avg_high_prio_sinr', 'sinr_5th_percentile', 'sinr_50th_percentile', 'sinr_95th_percentile']
    for scenario in df['scenario'].unique():
        scenario_df = df[df['scenario'] == scenario].copy()
        for metric in sinr_metrics_for_cdf:
            if metric not in scenario_df.columns: print(f"  Metric '{metric}' not found for CDF in {scenario}."); continue
            metric_data_for_plot = pd.to_numeric(scenario_df[metric], errors='coerce').dropna()
            if metric_data_for_plot.empty: print(f"  No valid data for CDF of '{metric}' in {scenario}."); continue
            plt.figure(figsize=(12,8)); legend_items_exist = False
            for algo in scenario_df['algorithm'].unique():
                for fuzzer in scenario_df['fuzzer_type'].unique():
                    ...
                    if not subset_data_numeric.empty:
                        ...
                        plt.plot(sorted_data, yvals, marker='.', linestyle='-', markersize=4, alpha=0.7, label=f'{algo} ({fuzzer})'); legend_items_exist = True
            if not legend_items_exist: plt.close(); print(f"  No data plotted for CDF of '{metric}' in {scenario}."); continue
            plt.title(f'CDF of {metric.replace("_", " ").title()} - Scenario: {scenario}'); plt.xlabel(metric.replace("_", " ").title() + " (dB where applicable)"); plt.ylabel('Cumulative Probability P(X <= x)')
            plt.legend(loc='best'); plt.grid(True, which="both", ls="--", linewidth=0.5)
            if "sinr" in metric.lower():
                 q_low = metric_data_for_plot.quantile(0.005); q_high = metric_data_for_plot.quantile(0.995)
                 q_low = -60 if pd.isna(q_low) else q_low; q_high = 70 if pd.isna(q_high) else q_high
                 buffer = (q_high - q_low) * 0.1 if (q_high - q_low) > 0 and pd.notna(q_high - q_low) else 10
                 final_min = max(q_low - buffer, -70 if "5th" in metric else -50); final_max = min(q_high + buffer, 70 if "95th" not in metric else 140)
                 if final_min >= final_max: final_min = metric_data_for_plot.min() -5 if not metric_data_for_plot.empty else -50; final_max = metric_data_for_plot.max() + 5 if not metric_data_for_plot.empty else (70 if "95th" not in metric else 140)
                 if final_min >= final_max: final_min = -50; final_max = (70 if "95th" not in metric else 140);
                 plt.xlim(final_min, final_max)
            plt.tight_layout(); plot_filename = os.path.join(output_dir, f"CDF_{metric}_{scenario.replace(' ','_')}.png")
...
def generate_summary_csv_with_flags(df, output_filename):
    ...
    print(f"\n--- Generating Summary CSV: {output_filename} ---"); summary_list = []
    ...
    if not all(col in df.columns for col in ['scenario', 'fuzzer_type', 'algorithm']): print("Error: Grouping columns missing."); return
    grouped = df.groupby(['scenario', 'fuzzer_type', 'algorithm'])
    for name, group in grouped:
        scenario, fuzzer_type, algorithm = name; row = {'scenario': scenario, 'fuzzer_type': fuzzer_type, 'algorithm': algorithm}
    ...
    try: summary_df.to_csv(output_filename, index=False, encoding='utf-8'); print(f"Summary CSV saved to: {output_filename}")
    except Exception as e: print(f"Error saving summary CSV: {e}")
    return summary_df

def print_diagnostic_flags_summary():
    # (Identical to version 5)
    print("\n--- Overall Diagnostic Flags Summary (from Analysis Script v5.2) ---") # Updated version
    ...
    if FLAGS["rsrp_values_suspiciously_high"]: print(f"      WARNING: RSRP values > {SUSPICIOUS_RSRP_THRESHOLD_DBM}dBm found for some UEs. This is suspicious and needs investigation in simulation logic.")
    if FLAGS["rsrp_values_at_floor"]: print("      INFO: Some UEs experienced RSRP at the floor value (-200 dBm), indicating extremely poor signal.")
    ...
    if not os.path.exists(target_csv):
        ...
        if os.path.exists(csv_from_last_good_run) and CSV_FILEPATH != csv_from_last_good_run :
            ...
            if not os.path.exists(CSV_FILEPATH):
                 ...
                 target_csv = None
    if target_csv:
        analysis_run_plot_dir = f"analysis_output_{os.path.splitext(os.path.basename(target_csv))[0]}_v5_3_run" # Updated version
