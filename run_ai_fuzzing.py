...
SCRIPT_VERSION_NAME = "v23_1_small_fast_fixed" # For output files
    ...
    for fuzzer_name, FuzzerClass in fuzzer_map.items():
        ...
        for actual_algo_name, ts_instance_proto in ts_prototypes.items():
            ...
            for iteration in range(SIMULATION_ITERATIONS):
                ...
                # --- FIX: Define pos_modifier_3d_np here ---
...
def main():
    ...
    gpus = tf.config.list_physical_devices('GPU'); strategy = tf.distribute.get_strategy() 
    if gpus:
        try:
            print(f"--- Physical GPUs detected: {[gpu.name for gpu in gpus]}. Configuring first GPU. ---")
            tf.config.set_visible_devices(gpus[0], 'GPU') 
            for gpu_device_config in tf.config.get_visible_devices('GPU'): tf.config.experimental.set_memory_growth(gpu_device_config, True)
            logical_gpus = tf.config.list_logical_devices('GPU')
            if logical_gpus: print(f"--- Configured to use {len(logical_gpus)} Logical GPU(s): {[lg.name for lg in logical_gpus]} ---")
            else: print("--- WARNING: No logical GPUs configured. ---")
        except Exception as e: print(f"Error during GPU setup: {e}. Using default CPU strategy.")
    else: print("--- No GPU detected by TensorFlow. Running on CPU. ---")
    with strategy.scope(): 
