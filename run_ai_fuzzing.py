...
class NetworkEnvironment:
    ...
    def update_ue_positions_and_velocities(self, dt=0.5, max_speed=None):
        ...
        new_loc_2d = new_loc[:, :, :2]
        bs_loc_2d = self.bs_loc[:, :, :2]
        new_loc_expanded = tf.expand_dims(new_loc_2d, axis=2)
        bs_loc_expanded = tf.expand_dims(bs_loc_2d, axis=1)
        distances = tf.norm(new_loc_expanded - bs_loc_expanded, axis=3)
        closest_bs_idx = tf.argmin(distances, axis=2)
        distance_to_closest = tf.reduce_min(distances, axis=2)
        mask = distance_to_closest > 200.0
        direction = new_loc_2d - tf.gather(bs_loc_2d, closest_bs_idx, batch_dims=1)
        norm_direction = direction / tf.maximum(distance_to_closest[:, :, tf.newaxis], 1e-9)
        new_loc_2d = tf.where(mask[:, :, tf.newaxis],
                             tf.gather(bs_loc_2d, closest_bs_idx, batch_dims=1) + norm_direction * 200.0,
                             new_loc_2d)
        new_loc = tf.concat([new_loc_2d, new_loc[:, :, 2:3]], axis=2)
        ...
        rsrp_db_np, _, _, _ = self.compute_metrics()
        rsrp_db_tf = tf.convert_to_tensor(rsrp_db_np, dtype=tf.float32)
        hysteresis_margin_db = 8.0
    ...
    def compute_metrics_tf(self, ue_loc_tf, bs_loc_tf, ut_orient_tf, bs_orient_tf, ut_vel_tf, in_state_tf):
        ...
        rsrp_db_tf = tf.where(tf.math.is_finite(rsrp_db_tf), rsrp_db_tf, -200.0)
        sinr_db_tf = tf.where(tf.math.is_finite(sinr_db_tf), sinr_db_tf, -30.0)
    ...
    def compute_metrics(self):
        ...
        try:
            ...
            rsrp_db_np = rsrp_db_tf.numpy()
            sinr_db_np = sinr_db_tf.numpy()
            if np.any(~np.isfinite(rsrp_db_np)) or np.any(~np.isfinite(sinr_db_np)):
                print("Warning: NaN or Inf values detected in rsrp or sinr. Replacing with defaults.")
                rsrp_db_np = np.where(np.isfinite(rsrp_db_np), rsrp_db_np, -200.0)
                sinr_db_np = np.where(np.isfinite(sinr_db_np), sinr_db_np, -30.0)
            if sinr_db_np.shape != (NUM_UES, NUM_CELLS):
                print(f"Error: sinr_db_np has incorrect shape {sinr_db_np.shape}, expected {(NUM_UES, NUM_CELLS)}")
                return np.full((NUM_UES, NUM_CELLS), -200.0), np.full((NUM_UES, NUM_CELLS), -30.0), \
                       self.cell_loads.copy(), self.ue_priorities.copy()
            return rsrp_db_np, sinr_db_np, self.cell_loads.copy(), self.ue_priorities.copy()
        except Exception as e:
    ...
    def update_cell_loads(self, assignments):
        ...
        assignments = np.clip(assignments, 0, NUM_CELLS-1)
...
class TrafficSteering:
    ...
    def assign_initial(self, rsrp):
        rsrp = np.where(np.isfinite(rsrp), rsrp, -200.0)
        assignments = np.argmax(rsrp, axis=1)
        assignments = np.clip(assignments, 0, NUM_CELLS-1)
    ...
    def baseline_a3(self, rsrp, sinr, cell_loads, priorities, dt):
        rsrp = np.where(np.isfinite(rsrp), rsrp, -200.0)
        sinr = np.where(np.isfinite(sinr), sinr, -30.0)
...
class AIFuzzer:
    ...
    def _objective_function(self, inputs, current_assignments, dt_fitness=1.0):
        ...
        try:
            load_modifier = np.clip(inputs[:NUM_CELLS], -0.2, 0.2)
            position_modifier_2d_np = np.clip(inputs[NUM_CELLS:].reshape(NUM_UES, 2), -10, 10)
            ...
            if not np.all(np.isfinite(rsrp)) or not np.all(np.isfinite(sinr)):
                print("Warning: NaN/Inf in rsrp or sinr in _objective_function")
                return -np.inf
            ...
            if not np.all((new_assignments >= 0) & (new_assignments < NUM_CELLS)):
                print(f"Invalid assignments detected: {new_assignments}")
                return -np.inf
            num_handovers = np.sum(new_assignments != current_assignments)
            sinr_selected = sinr[np.arange(NUM_UES), new_assignments]
            if not np.all(np.isfinite(sinr_selected)):
                print("Warning: NaN/Inf in selected SINR")
                return -np.inf
            fairness = self.oracle._jain_fairness(10**(np.clip(sinr_selected, -30, 30) / 10.0))
            fitness_score = -float(num_handovers) + 10 * fairness
        except Exception as e:
            print(f"Error in _objective_function: {e}")
    ...
    def generate_inputs(self, dt=1.0):
        ...
        for gen in range(self.generations):
            ...
            if not np.all(np.isfinite(fitness)):
                print(f"Warning: Non-finite fitness scores in generation {gen}")
        ...
        for actual_algo_name, ts_instance_proto in ts_prototypes.items():
            ...
            for iteration in range(SIMULATION_ITERATIONS):
                ...
                results_list.append({
                    ...
                    'cell_loads_list_str': str(shared_env_state.cell_loads.tolist()),
            ...
            for algo in fuzzer_data['algorithm'].unique():
                ...
                if 'vulnerabilities' in algo_fuzzer_df.columns:
                    ...
                    if not algo_vuln_strings:
                        ...
                        for vuln_str, count in algo_counts.most_common(5):
...
def main():
    script_version_name = "v23_fixed_index_error"
